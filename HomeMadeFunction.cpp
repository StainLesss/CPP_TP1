...
//I.1.3
// Polymorphisme
void updateArg3(int a,int b,int* c){// arg 3 = arg 1 +arg 2 using POINTEUR
    *c = a + b;
}

void updateArg3(int a,int b,int& c){// arg 3 = arg 1 +arg 2 using REFERENCE
    c = a + b;
}
...
void guessMyNumberPlay(){
    int TargetVal= randomFrom(0,10);
    int cnt=0;
    //cout << TargetVal <<endl;// Line for testing, provide the Target Value
    string usrVal;
    say("Nombre choisi ! Essaye de deviner !");
...
int guessMyNumberForPC(){
   ...
   int min = 0;
   int max = 1000;
   int moy = static_cast<int>((min+max)/2);
   return moy;
   //while (stayloop){
